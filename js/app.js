...
(() => {
  ...
  const defaultState = () => ({
    ...
    meals: [],       // { id, name, calories, proportion }
    ...
    ul.innerHTML = withData.map((s) => `
      <li class="day-row">
        <div class="day-head">
          ...
          <span class="day-bal ${s.balance > 0 ? "pos" : "neg"}">${signed(s.balance)}</span>
        </div>
      ...
      meals.innerHTML = state.meals.map((m) => `
        <li class="saved-row">
          <div class="saved-main">
            ...
            <div class="saved-meta">${fmt(m.calories)} kcal per ${trimNum(m.proportion)} unit${m.proportion == 1 ? "" : "s"} · ${trimNum(m.calories / m.proportion)}/unit</div>
          </div>
  ...
  function openEntryModal(kind) {
    ...
    const scaleLabel = isIntake ? "Proportion" : "Duration (min)";
    ...
    openModal(`
      ...
      <div id="tab-new">
        ...
        <div class="field hidden" id="save-scale-field">
          <label>${scaleLabel} this amount represents</label>
          <input type="number" id="new-scale" inputmode="decimal" value="1" step="any" />
          <p class="setting-hint">Later you can add any ${isIntake ? "proportion" : "duration"} and calories scale automatically.</p>
        </div>
    ...
    $("#confirm-new").addEventListener("click", () => {
      ...
      if ($("#save-toggle").checked) {
        ...
        if (isIntake) state.meals.push({ id: uid(), name, calories: cal, proportion: scale });
    ...
    if (saved.length) {
      ...
      $("#confirm-saved").addEventListener("click", () => {
        ...
        const unit = isIntake ? "" : " min";
        addEntry(kind, `${item.name} (${trimNum(q)}${unit})`, cal);
        closeModal();
      });
    }
  }
  ...
  function openSavedModal(kind, existing) {
    ...
    const scaleLabel = isMeal ? "Base proportion" : "Base duration (min)";
    ...
    $("#lib-save").addEventListener("click", () => {
      ...
      if (isMeal) {
        if (existing) Object.assign(existing, { name, calories: cal, proportion: scale });
        else state.meals.push({ id: uid(), name, calories: cal, proportion: scale });
      } else {
